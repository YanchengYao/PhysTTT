...
def train_and_test(config, data_loader_dict):
    ...
    elif config.MODEL.NAME == 'PhysioLSTM':
        model_trainer = trainer.PhysioLSTMTrainer.PhysioLSTMTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'PhysTTT':
        model_trainer = trainer.PhysTTTTrainer.PhysTTTTrainer(config, data_loader_dict)
    else:
...
if __name__ == "__main__":
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        elif config.TRAIN.DATA.DATASET == "HCI":
            train_loader = data_loader.HCILoader.HCILoader
        else:
        ...
        if config.VALID.DATA.DATASET == "COHFACE":
            ...
            valid_loader = data_loader.HCILoader.HCILoader
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        ...
        if config.TEST.DATA.DATASET == "COHFACE":
            ...
            test_loader = data_loader.HCILoader.HCILoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), and UBFC-PHYS.")
