...
class DeepPhys(nn.Module):
    ...
    def __init__(self, in_channels=3, nb_filters1=32, nb_filters2=64, kernel_size=3, dropout_rate1=0.25,
                 dropout_rate2=0.5, pool_size=(2, 2), nb_dense=128, img_size=128):
        """Definition of DeepPhys.
        Args:
          in_channels: the number of input channel. Default: 3
          img_size: height/width of each frame. Default: 128 //36. //modified
        Returns:
          DeepPhys model.
        """
    ...
    def forward(self, inputs, params=None):
        ...
        x_t_1 = torch.cat([inputs[:, 1:, :, :, :], inputs[:, D - 1:, :, :, :]], 1)
        diff_input = (x_t_1 - inputs) / (x_t_1 + inputs)
        diff_input = diff_input.view(N * D, C, H, W)
        raw_input = inputs.view(N * D, C, H, W)
        # diff_input = inputs[:, :3, :, :]
        # raw_input = inputs[:, :3, :, :]  #inputs[:, 3:, :, :]
