...
# from ttt.models.cogvideo.utils import (SequenceMetadata, full_tensor, place_into, shard_tensor, to_local)
...
from neural_methods.model.base.ops import ttt_linear, ttt_mlp
from neural_methods.model.base.utils import apply_rotary_emb, precompute_freqs_cis_3d


class TTTLayer(nn.Module):
    def __init__(self, model_dim, num_heads, method, mini_batch_size, ttt_base_lr, scan_checkpoint_group_size):
        super().__init__()
        ...
        if self.method == "ttt_linear":
            self.ttt = TTTLinear()
        elif self.method == "ttt_mlp":
            self.ttt = TTTMLP(self.model_dim, self.num_heads, self.mini_batch_size, self.ttt_base_lr, self.scan_checkpoint_group_size)
        else:
            raise TypeError(f"No ttt layer of type {self.method}")
...
class TTTBase(nn.Module):
    def __init__(self, model_dim, num_heads, mini_batch_size, ttt_base_lr, scan_checkpoint_group_size):
        super().__init__()
    ...
    def forward(self, hidden_states: torch.Tensor):
        assert (
            hidden_states.size(1) % self.mini_batch_size == 0
        ), "Sequence len must be multiple of mini batch size."

        hidden_states = self.ttt(self.process_input(hidden_states))
