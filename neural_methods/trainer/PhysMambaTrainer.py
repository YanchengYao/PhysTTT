...
from neural_methods.loss import TorchLossComputer
...
class PhysMambaTrainer(BaseTrainer):
    ...
    def intro_valid(self, data_loader):
        """ Runs the model on valid sets."""
        if data_loader["valid"] is None:
        ...
        self.model.eval()
        print("Running model evaluation on the valid dataset!")
        with torch.no_grad():
            vbar = tqdm(data_loader["valid"], ncols=80)
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                for idx in range(batch_size):
                    subj_index = valid_batch[2][idx]
                    sort_index = int(valid_batch[3][idx])
