...
from neural_methods.loss.TorchLossComputer import Hybrid_Loss, mae_loss, rmse_loss, Neg_Pearson
...
class RhythmMambaTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            self.criterion = Hybrid_Loss()  # Hybrid_Loss()
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # print(f"Data device: {data.device}, dtype: {data.dtype}")
                pred_ppg = self.model(data)
                # pred_ppg = pred_ppg.to(self.device)
                pred_ppg = (pred_ppg-torch.mean(pred_ppg, axis=-1).view(-1, 1))/torch.std(pred_ppg, axis=-1).view(-1, 1)    # normalize

                loss = 0.0
                for ib in range(N):
                    loss = loss + self.criterion(pred_ppg[ib], labels[ib], epoch , self.config.TRAIN.DATA.FS , self.diff_flag)  #加了.view(1,-1) 要删除
                loss = loss / N
                loss.backward()
                self.optimizer.step()
                self.scheduler.step()
