...
from neural_methods.model.PhysTTT import PhysTTT
from neural_methods.trainer.BaseTrainer import BaseTrainer
from neural_methods.loss.TorchLossComputer import PhysTTT_Loss
import time

class PhysTTTTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        self.output_save_dir = config.TEST.OUTPUT_SAVE_DIR

        if config.TRAIN.DATA.PREPROCESS.LABEL_TYPE == "DiffNormalized":
            self.diff_flag = 1
        if config.TOOLBOX_MODE == "train_and_test":
            self.model = PhysTTT().to(self.device)
            self.model = torch.nn.DataParallel(self.model, device_ids=list(range(config.NUM_OF_GPU_TRAIN)))
            self.num_train_batches = len(data_loader["train"])
            self.criterion = PhysTTT_Loss()
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                for ib in range(N):
                    ...
                    save_preds.append(pred_ppg[ib])
                    save_labels.append(labels[ib])
                loss = loss / N
                loss.backward()
                self.optimizer.step()
                self.scheduler.step()
    ...
    def test(self, data_loader):
        ...
        self.model = self.model.module.to(self.config.DEVICE)
        self.model.eval()
        with torch.no_grad():
            predictions = dict()
            labels = dict()
            for _, test_batch in enumerate(data_loader['test']):
                ...
                t2 = time.time()*1000
                num = pred_ppg_test.shape[0]
                print("Testing took {} milliseconds".format((t2 - t1)/num))
            ...
            print('Save predictions and labels in path: ', self.output_save_dir)
            np.save(self.output_save_dir + '/predictions.npy', predictions)
            np.save(self.output_save_dir + '/labels.npy', labels)
        ...
        for idx in range(N):
            ...
            if rand1 < 0.5:
                ...
                if gt_hr_fft > 90:
                    ...
                    data_aug[:, odd_indices, :, :, :] = (data[:, rand3 + odd_indices // 2, :, :, :] + data[:, rand3 + (
                                odd_indices // 2) + 1, :, :, :]) / 2
                    labels_aug[:, odd_indices] = (labels[:, rand3 + odd_indices // 2] + labels[:, rand3 + (
                                odd_indices // 2) + 1]) / 2
                elif gt_hr_fft < 75:
                    data_aug[:, :D // 2, :, :, :] = data[:, ::2, :, :, :]
