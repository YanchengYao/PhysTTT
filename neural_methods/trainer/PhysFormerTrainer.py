...
class PhysFormerTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            self.model = ViT_ST_ST_Compact3_TDC_gra_sharp(image_size=(300,128,128), patches=(4,4,4), dim=96, ff_dim=144, num_heads=4, num_layers=12, dropout_rate=0.1, theta=0.7).to(self.device)
    ...
    def test(self, data_loader):
        ...
        self.model = self.model.module.to(self.config.DEVICE)  # module
        self.model.eval()
        with torch.no_grad():
            predictions = dict()
            labels = dict()
