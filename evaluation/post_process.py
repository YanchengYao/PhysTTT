...
def get_hr(y, sr=30, min=45, max=150): # 45 150
    p, q = welch(y, sr, nfft=1e5/sr, nperseg=np.min((len(y)-1, 256)))
    # print(p[(p>min/60)&(p<max/60)])
    # print(q[(p>min/60)&(p<max/60)])
    # print(np.argmax(q[(p>min/60)&(p<max/60)]))
...
def _calculate_peak_hr(ppg_signal, fs):
    ...
    ppg_peaks, _ = scipy.signal.find_peaks(ppg_signal, distance=12)  #, distance=24
