...
def calculate_metrics(predictions, labels, config):
    ...
    if config.INFERENCE.EVALUATION_METHOD == "FFT":
        ...
        for metric in config.TEST.METRICS:
            ...
            elif metric == "Pearson_coff":
                Pearson_coff_FFT = np.mean(pearson_corr_all)
                standard_error = np.std(Pearson_coff_FFT) / np.sqrt(num_test_samples)
                print("FFT Pearson_coff (FFT Label): {0} +/- {1} (dB)".format(Pearson_coff_FFT, standard_error))
            elif "AU" in metric:
                pass
            elif "BA" in metric:  
